...
**Proof Of Work**, 작업을 통해 증명을 한다는 뜻이다. 여기서 말하는 '작업'이란 해시값을 구하는 것을 말한다. 즉, 우리가 블록을 블록체인에 올리고 싶다면 해시값을 찾는 일종의 문제를 풀어야 하는 과정을 거쳐야 하는 것이고, 이 과정을 거쳐서 블록이 확정 상태가 되었다면 블록에 있는 거래는 ***믿을 수 있는 것***이 된다.
...
_*다수결에 의한 결정은 가장 긴 체인으로 나타내어지며 이는 가장 많은 작업증명 노력이 들어간 체인이다. 과반수의 컴퓨팅 파워가 정직한 노드에 의해 제어된다면 정직한 체인이 가장 빨리 길어져 다른 경쟁 체인들을 앞서게 될 것이다.*_
