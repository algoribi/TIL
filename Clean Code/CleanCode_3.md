...
```
첫째, 새 직원의 유형이 추가될 때마다 코드가 더 길어질 것이다.
둘째, '한 가지' 작업만 수행하지 않는다.
셋째 SRP(단일 책임 원칙)를 위반한다. 코드를 변경할 이유가 여럿이기 때문이다.
넷째, OCP(개방 폐쇄 원칙)를 위반한다. 새 직원 유형을 추가할 때마다 코드를 변경하기 때문이다.
마지막으로 위 함수와 동일한 구조의 함수가 무한정 존재할 수 있다는 사실이다.
```
