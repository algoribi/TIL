...
- **Q.** 위의 절차적인 코드 구조도 실제 현업에서 많이 사용되는지?
...
- **Q.** 단순한 코드 질문. area()는 인터페이스를 통해 오버라이드 하는 메서드인데, 예제 코드에서는 어째서 함수 앞에 @override 어노테이션을 붙이지 않는 것인지? (다른 이유가 있는지?) 뒤의 10장의 예제에서 추상 클래스에서 메서드를 오버라이딩할 때는 @override를 사용.
...
- **Q.** 만약 (공부의 경우가 아닌 현업에서)코드를 읽으면서 코드 작성자가 추상화를 해 놓은 코드(ex. [3장](https://github.com/algoribi/TIL/blob/main/Clean%20Code/CleanCode_3.md)의 EmployeeFactoryImpl 같은)가 있다면 그 부분은 읽지 않고 넘어가도 되는가? 추상화는 어쨌든 자신이 짠 코드를 상대가 보지 않아도 되는 경우에 하는 것이니까?
